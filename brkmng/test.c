...
int main(int argc, char *argv[])
{
   ...
   //strcpy(connection->remote_ip, "127.0.0.1");
   rc=stomp_connect( &connection, "127.0.0.1", 61613, pool);
   ...
   fprintf(stdout, "Sending Subscribe.");
   {
      ...
      frame.command = "SUB";
      frame.headers = apr_hash_make(pool);
      apr_hash_set(frame.headers, "destination", APR_HASH_KEY_STRING, "/queue/TEST.FOO");      
	  frame.body_length = -1;
      frame.body = NULL;
      rc = stomp_write(connection, &frame, pool);
      rc==APR_SUCCESS || die(-2, "Could not send frame", rc);
